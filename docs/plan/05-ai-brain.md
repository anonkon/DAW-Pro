...
band-agnostic ones. Critically, it does **not** compute a resolved/ignored
...
loose rhythmic cohesion, with thresholds documented in `retrieval.py` and
flagged as heuristic where they aren't from a cited source). Skill-tier
pitfall chunks only ride along when their paired technique trigger fires
*and* the persona is at the matching tier, so they're never generic filler.

This mirrors the spec's own description of the retrieval query, which says
it should be "built from the combination of detected problems and user
genre, not from the user's words alone." That describes structured signals,
not free text to embed. It also keeps the same discipline as the measurement
guardrail and Layer 2's history: `knowledge/base.py`'s chunks each cite a
`source` (a real reference, not an invented number) or say `"heuristic"`
when the number is our own threshold choice, and the system prompt
explicitly tells Gemini `knowledge_context` is background, not a fact about
the user's own audio. Only `measurements` is that.

**On prompt order**: the spec's stated order is audio features, then user
profile/history, then retrieved knowledge, then system instruction. The
system instruction is necessarily its own leading `SystemMessage` in the
Gemini/LangChain call (not reorderable to last), so `human_payload`'s
`HumanMessage` carries the three data layers in the stated order instead.
Same intent (ground the model in data before it reasons), adapted to how a
system-message API actually works.
...
- **K-weighting**: the exact two-stage IIR filter from BS.1770-5 Annex 1
  Tables 1-2 (head-effect shelf plus RLB high-pass), applied at the
  coefficients' specified 48kHz (audio is resampled to 48kHz first, which the
  spec explicitly permits over re-deriving coefficients for other rates).
...
- **`Mixing_Techniques_for_Audio_-_FINAL.pdf`** (a Routledge chapter sampler,
  where the useful material is almost entirely Wessel Oltheten's "Advanced
  ...
  vinyl-cutting mono-bass precedent), and added `technique_thin_low_end` and
...
- **`The_Art_of_Mastering_in_Music_-_Final.pdf`** (another chapter sampler)
  ...
  and `skill_beginner_overcompression` with concrete mastering-limiter
  discipline (keep gain reduction to 3-4dB per stage, since over-reliance on
...
- **`pdfcoffee.com_dance-music-manual-...pdf`** turned out to be a truncated,
  ...
  and so on, all listed in the TOC) are actually present in this copy, so it
...
- **`toaz.info-the-mixing-engineerx27s-handbook-...pdf`** (Bobby Owsinski,
  ...
  was the richest single source. It added `genre_acoustic` (LUFS-by-genre
  table: acoustic/organic material commonly -12 to -14 LUFS, more dynamic
  range than electronic/pop). It deepened `technique_low_mid_mud` with the
  ...
  directly. The amateur-mix list confirmed verbatim but wasn't used, since
  it's a holistic checklist that doesn't map to any single measured trigger,
  and including it would violate the "no untriggered generic advice" rule
  this knowledge base has held throughout. The panning claim wasn't found in
  the page range checked and was dropped rather than guessed at.
- **`document (1).pdf`** (*Making Sound*, Cristofer Odqvist) was the
  ...
  figures for `technique_transient_attack` (slow attack of 30ms or more
  reads brighter and more energetic, fast attack around 3ms darkens and
  mellows, since attack time controls perceived brightness because
  brightness lives mostly in the transient). Also deepened
  `technique_stereo_bass_mono` with a second, looser mono-bass threshold
  (the book gives two different numbers, 80Hz as the strict perceptual
  localization limit and 150Hz as a looser practical rule of thumb many
  mixers use, and both are kept rather than picking one, per the book's own
  distinction).
- **`feismo.com-music-theory-for-computer-musicians-...pdf`** (Michael
  ...
  for key compatibility and clash, relative major/minor as the closest
  relationship, and a properly-hedged sharp-is-brighter/flat-is-darker note,
  since the book itself flags that as a subjective convention among
  musicians, not an acoustic fact, and the chunk preserves that hedge rather
  than stating it as settled). This needed a small schema change first:
  `Measurements` only carried the *project's* detected key, not the
  reference's, so there was nothing to compare. `reference_key` and
...
agent pipelines. That's a mismatch for "call one LLM with structured output,
let the caller pick the provider." OpenAI is also winding down Agent Builder
and Evals from November 30, 2026. The existing LangChain-based approach
already had the right shape. It just needed extending to more providers, not
...
so `analyze()` treats a missing `OPENAI_MODEL` the same as a missing API key
and falls back to the existing "measurements without AI narrative" path,
rather than risking a call to a wrong or nonexistent model string. Gemini's
